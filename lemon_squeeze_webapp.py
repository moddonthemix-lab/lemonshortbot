...
@app.route('/app')
def app_shell():
    """Serve legacy all-in-one app shell"""
    html_files = [
        'lemon_squeeze_webapp.html',
        'lemon_squeeze_complete_with_chat.html',
        'lemon_squeeze_with_volemon__4_.html',
    ...
    for html_file in html_files:
        if os.path.exists(html_file):
            return send_from_directory('.', html_file)
    return "<h1>🍋 Lemon Squeeze App Shell Not Found</h1>"

# Tool landing pages (reuse legacy app via iframe/query param)
TOOL_ROUTES = {
    'squeeze': {
        'title': 'Short Squeeze',
        'query': 'squeeze',
        'tab': 'squeeze',
        'subtitle': 'Top short interest candidates with risk and volume pressure.'
    },
    'daily': {
        'title': 'Daily Plays',
        'query': 'daily',
        'tab': 'daily',
        'subtitle': 'Daily 3-1 Strat scans with news and volume context.'
    },
    'hourly': {
        'title': 'Hourly Plays',
        'query': 'hourly',
        'tab': 'hourly',
        'subtitle': 'Intraday hourly patterns for day-traders.'
    },
    'weekly': {
        'title': 'Weekly Plays',
        'query': 'weekly',
        'tab': 'weekly',
        'subtitle': 'Weekly 3-1 setups for swing bias.'
    },
    'volemon': {
        'title': 'Volemon',
        'query': 'volemon',
        'tab': 'volemon',
        'subtitle': 'High volume moves across the core list.'
    },
    'usuals': {
        'title': 'Usuals',
        'query': 'usuals',
        'tab': 'usuals',
        'subtitle': 'Staple tickers with pattern and volume context.'
    },
    'crypto': {
        'title': 'Crypto Plays',
        'query': 'crypto',
        'tab': 'crypto',
        'subtitle': 'BTC, ETH, and friends across 1h/4h/daily/weekly.'
    },
    'lemonai': {
        'title': 'LemonAI',
        'query': 'lemonai',
        'tab': 'lemonai',
        'subtitle': 'Options-aware AI with flow, confidence, and strikes.'
    },
    'challenges': {
        'title': 'Challenges',
        'query': 'challenges',
        'tab': 'challenges',
        'subtitle': 'Weekly/monthly trading challenges and progress.'
    },
    'lemonade': {
        'title': 'Lemonade',
        'query': 'lemonade',
        'tab': 'lemonade',
        'subtitle': 'Dividends and passive pours.'
    },
    'howto': {
        'title': 'How to Use',
        'query': 'howto',
        'tab': 'howto',
        'subtitle': 'Playbook, onboarding, and quick FAQs.'
    }
}

def render_tool_page(tool_key):
    meta = TOOL_ROUTES.get(tool_key)
    if not meta:
        return redirect(url_for('index'))
    iframe_src = f"/app?tool={meta['query']}"
    return render_template(
        ...
        tool_key=tool_key,
        tab_key=meta.get('tab', meta['query'])
    )

@app.route('/squeeze')
def squeeze_page():
    # Serve redesigned squeeze page (no iframe)
    meta = TOOL_ROUTES.get('squeeze')
    return render_template('tool_squeeze.html', title=meta['title'], subtitle=meta.get('subtitle', ''))

@app.route('/daily')
def daily_page():
    meta = TOOL_ROUTES.get('daily')
    return render_template(
        'tool_generic.html',
        title=meta['title'],
        subtitle=meta.get('subtitle', ''),
        tool_key='daily',
        api_endpoint='/api/daily-plays',
        api_method='POST'
    )

@app.route('/hourly')
def hourly_page():
    meta = TOOL_ROUTES.get('hourly')
    return render_template(
        'tool_generic.html',
        title=meta['title'],
        subtitle=meta.get('subtitle', ''),
        tool_key='hourly',
        api_endpoint='/api/hourly-plays',
        api_method='POST'
    )

@app.route('/weekly')
def weekly_page():
    meta = TOOL_ROUTES.get('weekly')
    return render_template(
        'tool_generic.html',
        title=meta['title'],
        subtitle=meta.get('subtitle', ''),
        tool_key='weekly',
        api_endpoint='/api/weekly-plays',
        api_method='POST'
    )

@app.route('/crypto')
def crypto_page():
    meta = TOOL_ROUTES.get('crypto')
    return render_template(
        'tool_generic.html',
        title=meta['title'],
        subtitle=meta.get('subtitle', ''),
        tool_key='crypto',
        api_endpoint='/api/crypto-plays',
        api_method='POST'
    )

@app.route('/lemonai')
def lemonai_page():
    meta = TOOL_ROUTES.get('lemonai')
    return render_template(
        'tool_generic.html',
        title=meta['title'],
        subtitle=meta.get('subtitle', ''),
        tool_key='lemonai',
        api_endpoint='/api/lemonai-analyze',
        api_method='POST'
    )

@app.route('/volemon')
def volemon_page():
    meta = TOOL_ROUTES.get('volemon')
    return render_template(
        'tool_generic.html',
        title=meta['title'],
        subtitle=meta.get('subtitle', ''),
        tool_key='volemon',
        api_endpoint='/api/volemon-scan',
        api_method='POST'
    )

@app.route('/usuals')
def usuals_page():
    meta = TOOL_ROUTES.get('usuals')
    return render_template(
        'tool_generic.html',
        title=meta['title'],
        subtitle=meta.get('subtitle', ''),
        tool_key='usuals',
        api_endpoint='/api/usuals-scan',
        api_method='POST'
    )

@app.route('/challenges')
def challenges_page():
    meta = TOOL_ROUTES.get('challenges')
    return render_template(
        'tool_generic.html',
        title=meta['title'],
        subtitle=meta.get('subtitle', ''),
        tool_key='challenges',
        api_endpoint='/api/challenges',
        api_method='GET'
    )

@app.route('/lemonade')
def lemonade_page():
    meta = TOOL_ROUTES.get('lemonade')
    return render_template(
        'tool_generic.html',
        title=meta['title'],
        subtitle=meta.get('subtitle', ''),
        tool_key='lemonade',
        api_endpoint='/api/dividend-data',
        api_method='GET'
    )

@app.route('/howto')
def howto_page():
    meta = TOOL_ROUTES.get('howto')
    return render_template(
        'howto.html',
        title=meta['title'],
        subtitle=meta.get('subtitle', '')
...
def scan():
    ...
    try:
        ...
        cache_key = f"squeeze_{min_short}_{min_gain}_{min_vol_ratio}_{min_risk}"
...
def daily_plays():
    ...
    try:
        ...
        cache_set('daily_plays', payload, ttl=300)
        return jsonify(payload)
...
def weekly_plays():
    ...
    try:
        ...
        cache_set('weekly_plays', payload, ttl=1800)
        return jsonify(payload)
...
def hourly_plays():
    ...
    try:
        ...
        cache_set('hourly_plays', payload, ttl=240)
        return jsonify(payload)
...
def crypto_plays():
    ...
    try:
        ...
        payload = {'success': True, 'results': results}
        cache_set('crypto_plays', payload, ttl=300)
        return jsonify(payload)
...
def volemon_scan():
    ...
    try:
        ...
        payload = {'success': True, 'results': results[:50]}
        cache_set('volemon_scan', payload, ttl=300)
        return jsonify(payload)
...
def usuals_scan():
    ...
    try:
        ...
        payload = {'success': True, 'results': results}
        cache_set('usuals_scan', payload, ttl=300)
        return jsonify(payload)
...
def lemonai_analyze():
    ...
    try:
        cached_resp = cache_get('lemonai_analyze')
        if cached_resp:
            return jsonify(cached_resp)
        ...
        cache_set('lemonai_analyze', payload, ttl=300)
        return jsonify(payload)
...
def get_challenges():
    ...
    try:
        ...
        cache_set('challenges', payload, ttl=1800)
        return jsonify(payload)
...
def get_dividend_data():
    ...
    try:
        cached = cache_get('dividend_data')
        if cached:
            return jsonify(cached)
        ...
        cache_set('dividend_data', payload, ttl=1800)
        return jsonify(payload)
